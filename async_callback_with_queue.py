...
async def run_ble_client(args: argparse.Namespace, queue: asyncio.Queue):
    ...
    if args.address:
        device = await BleakScanner.find_device_by_address(
            args.address, cb=dict(use_bdaddr=args.macos_use_bdaddr), timeout=60
        )
        if device is None:
        ...
        device = await BleakScanner.find_device_by_name(
            args.name, cb=dict(use_bdaddr=args.macos_use_bdaddr), timeout=120
        )
        if device is None:
...
async def run_queue_consumer(queue: asyncio.Queue):
    ...
    while True:
        ...
        if data is None:
            ...
            print("batteryLevel: {}%".format(batteryLevel))
